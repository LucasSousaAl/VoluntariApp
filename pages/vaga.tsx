...
export default function VagaPage() {
  ...
  const handleShare = async () => {
    ...
    try {
      const shareUrl = `${window.location.origin}/vaga?id=${vaga.id}`;

      if (navigator.share) {
        await navigator.share({
          title: vaga.title,
          text: `Confira esta vaga de voluntariado da ${vaga.ong}: ${vaga.title}`,
          url: shareUrl,
        });
      } else {
      ...
      message.error("Não foi possível compartilhar a vaga.");
    }
  };
